...
from cap.modules.git.errors import GitError
from cap.modules.git.factory import create_git_api
from cap.modules.git.tasks import download_repo, download_repo_file
from cap.modules.git.utils import create_webhook, parse_git_url
...
class CAPDeposit(Deposit):
    ...
    def upload(self, pid=None, *args, **kwargs):
        ...
        Expects json with params `url`,`webhook` (true|false), `event_type` (release|push)'.
        """
        with UpdateDepositPermission(self).require(403):
            if request:
                ...
                # TOFIX should be passed from the UI
                ...
                try:
                    host, owner, repo, branch, filepath, _ = \
                            parse_git_url(url)
